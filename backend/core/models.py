...
class Notification(Base):
    ...
    def __repr__(self):
        return f"<Notification(title='{self.title[:20]}', read={self.is_read})>"
...
class UserPreference(Base):
    ...
    __table_args__ = (
        UniqueConstraint("product_id", name="uq_user_pref_product"),
    )

    def __repr__(self):
        return f"<UserPreference(product_id={self.product_id})>"
